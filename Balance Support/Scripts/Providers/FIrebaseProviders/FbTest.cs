...
public class FbTest
{
    ...
    {
        var users = await firebaseClient.Child("Users").OrderBy("DisplayName").EqualTo("testuser6")
            .OnceAsync<UserAuthData>();

        var user = await firebaseClient.Child("Users").PostAsync(new UserAuthData()
            { Id = "aaaaa", Email = "aaaaa@gmail.com", DisplayName = "aaaaaUser" });
